...
gantt
    ...
    VoltDB integration             :done, p3, after p2, 1d
...
- Backend: FastAPI (натурально интегрируется с pandas);
- Frontend: React + D3.js или Cytoscape для графа;
- WebSocket для прогресса 80-секундного расчёта.
...
- Опциональный фильтр по `STATE` из VoltDB (исключить «закрытые»);
- Шаблоны имён (например, исключить всё, что начинается с «Company »
  или содержит «Тест»).

Реализация — отдельный модуль `filters.py`, применяется в `build_graph()`.
...
### 2.1. REST API + аутентификация

Вынести логику в API-слой. Эндпоинты:

| Endpoint | Что делает |
|---|---|
| `POST /api/recompute` | Запустить полный пересчёт (async, возвращает job_id) |
| `GET /api/job/{id}` | Статус расчёта |
| `GET /api/company/{bin}` | Профиль компании по последнему расчёту |
| `GET /api/aggregate` | Глобальный агрегат |
| `GET /api/list-cases` | Меню архетипов |

Авторизация: SSO через корпоративный IDP.
...
- [ ] Type hints во всех модулях — есть только в `kz_index.py` и `volt_resolver.py`;
- [ ] Логирование вместо `print()`;
- [ ] CI/CD — на момент прототипа `.github/workflows/` пуст;
- [ ] Schema validation для `.env` через `pydantic-settings`;
- [ ] Health checks для VoltDB и MySQL подключений;
...
| Когда планируется унификация формата BIN (БИН/ИИН) в MySQL до VARCHAR(12)? | DBA источника ЭСФ |
