...
Все эти представления реализованы в `kz_index.py`.

## Профиль компании { #target-card }

`_print_target_card()` печатает базовые метрики по узлу.
...
`show_backward_view()` — обход графа **вверх** от заданного BIN, отвечает
...
`show_forward_view()` — обход графа **вниз**, отвечает на вопрос:
...
`show_aggregate_economy()` печатает глобальные метрики:
...
`list_cases()` сканирует граф и предлагает по 5 кандидатов в каждой из
